...
faixaIdoso x = if (x>=60 && x<=64) then "IDO64" else if (x>=65 && x<=69) then "IDO69" 
else if (x>=70 && x<=74) then "IDO74" else if (x>=75 && x<=79) then "IDO79" 
else if (x>=80) then "IDO80" else "ND" 
...
classifIdosos listatupla = [(nome, idade, faixaIdoso idade) | (nome, idade) <- listatupla]
...
classifIdosos' :: [(String,Int)] -> [(String,Int,String)]
classifIdosos' listatupla = map (\(x,y) -> (x, y, faixaIdoso y)) listatupla
